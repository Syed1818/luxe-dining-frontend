...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    ...
    // Restore status if page was refreshed
  ...
  // 2. Real-time Connection (with local storage syncing)
  useEffect(() => {
    ...
    connection.on("OrderPreparing", (data) => {
      if (data.orderID?.toString() === id.toString()) {
        setOrderStatus(1);
        localStorage.setItem(`orderStatus_${id}`, "1"); // Save to memory
      }
    });
    ...
    connection.on("OrderReady", (data) => {
      if (data.orderID?.toString() === id.toString() || data.tableID?.toString() === sessionData.table.toString()) {
        setOrderStatus(2);
        setEtaMinutes(0);
        localStorage.setItem(`orderStatus_${id}`, "2"); // Save to memory
      }
    });
  ...
  const handleDownloadReceipt = () => {
    window.print(); // Triggers the browser's native Save-to-PDF / Print engine
  };
  ...
  return (
    <>
      {/* Cinematic Background (Hidden when printing PDF) */}
      ...
      {/* MAIN TRACKING UI (Hidden when printing PDF) */}
