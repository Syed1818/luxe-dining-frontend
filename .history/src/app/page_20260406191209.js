...
export default function CustomerMenu() {
  ...
  // 1. Fetch LocalStorage Data on Load
  useEffect(() => {
    // This safely runs only in the browser, avoiding Next.js server-side crashes
  ...
  // 3. Listen for real-time Kitchen updates!
  useEffect(() => {
    // Wait until we know the table number before connecting
    ...
    connection.on("OrderReady", (data) => {
      // Safely compare as strings just in case
      if (data.tableID.toString() === sessionData.table.toString()) {
        alert(`🔔 DING! ${sessionData.name}, your Order #${data.orderID} is ready for collection!`);
      }
    });
  ...
  }, [sessionData.table, sessionData.name]); // Re-run this effect when sessionData loads
  ...
  const submitOrder = async () => {
    ...
    if (sessionData.table === "Unknown") return alert("Please scan a table QR code to check in first!");

    const orderPayload = {
      // Convert the string table number back to an INT for the C# backend
    ...
    try {
      ...
      if (response.ok) {
        alert("Order placed successfully! We will alert you when it is ready.");
        setCart([]); // Clear cart
      } else {
      ...
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        ...
        <div className="text-center mb-6 border-b pb-4">
           ...
           <p className="text-gray-600">
             Welcome, <span className="font-bold text-blue-600">{sessionData.name}</span> (Table {sessionData.table})
           </p>
        </div>
