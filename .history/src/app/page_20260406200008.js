...
export default function CustomerMenu() {
  ...
  useEffect(() => {
    ...
    connection.on("OrderReady", (data) => {
      if (data.tableID.toString() === sessionData.table.toString()) {
        // TOAST: Order Ready Alert
        toast.success(`${sessionData.name}, your order is ready for collection!`, { icon: '🛎️', duration: 8000 });
      }
    });
  ...
  const submitOrder = async () => {
    if (cart.length === 0) return toast.error("Your cart is empty! Please add some items.");
    ...
    try {
      ...
      if (res.ok) {
        // TOAST: Success
        toast.success("Order placed! Your receipt has been emailed to you.", { duration: 5000 });
        setCart([]); 
      } else {
        toast.error("Something went wrong placing your order.");
      }
    } catch (err) {
      toast.error("Network error. Could not connect to the restaurant.");
    }
  };
