...
export default function CatalogMenu() {
  ...
  const submitOrder = async () => {
    ...
    // Group identical items together for a clean receipt!
    const groupedItems = {};
    cart.forEach(item => {
      ...
      if (groupedItems[id]) {
        ...
        groupedItems[id] = { itemID: id, quantity: 1 };
      }
    });
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Trigger the stunning success modal
        setSuccessModalData({
          orderId: newOrderId,
          ...
          <div className="flex-grow">
            <motion.div layout variants={containerVariants} initial="hidden" animate="show" key={selectedCategory} className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              <AnimatePresence mode="popLayout">
                {menuItems.length === 0 && <p className="text-zinc-500 italic p-4">Loading catalog...</p>}
                
                {filteredItems.map((item) => (
                  <motion.div layout variants={cardVariants} key={getItemId(item)} transition={fluidSpring} className="group flex bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden hover:bg-white/[0.08] hover:border-white/30 transition-colors duration-500 h-40 shadow-lg">
