...
const API_BASE_URL = "https://localhost:7170"; // Make sure this is your exact port!

export default function KitchenDisplay() {
  const [orders, setOrders] = useState([]);
  const [menuDict, setMenuDict] = useState({});
  const [connectionStatus, setConnectionStatus] = useState("Connecting...");
  const [loadingOrderId, setLoadingOrderId] = useState(null); // Tracks which button is clicked
  const connectionRef = useRef(null);

  useEffect(() => {
    // 1. Fetch Menu Dictionary to translate Item IDs to Names
    ...
    // 2. NEW: Fetch existing active orders so they don't vanish on refresh!
    fetch(`${API_BASE_URL}/api/Orders/active`)
      .then(res => res.json())
      .then(data => setOrders(data))
      .catch(err => console.error("Failed to fetch active orders:", err));

    // 3. Setup Real-Time SignalR Connection
    if (!connectionRef.current) {
      ...
      newConnection.on("ReceiveNewOrder", (newOrder) => {
        setOrders(prev => [...prev, newOrder]); // Add new orders to the end of the line
      });
  ...
  // 4. Fixed Mark as Ready Logic
  const completeOrder = async (orderId) => {
    setLoadingOrderId(orderId); // Show visual feedback that it's working
    ...
    try {
      ...
      if (res.ok) {
        // Remove the ticket smoothly using Framer Motion
        setOrders(prevOrders => prevOrders.filter(o => o.orderID !== orderId));
      } else {
        alert(`Failed to complete order. Server returned: ${res.status}`);
      }
    } catch (err) {
      alert("Network error trying to complete order. Is the backend running?");
    } finally {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-6 md:p-10 font-sans selection:bg-amber-500/30">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-zinc-900 border border-zinc-800 p-6 rounded-2xl shadow-xl mb-10">
        ...
        <div className={`mt-4 md:mt-0 px-4 py-2 rounded-full border text-sm font-bold tracking-wider ${
        ...
        <AnimatePresence>
          {orders.map((order, index) => (
            <motion.div 
              ...
              exit={{ opacity: 0, scale: 0.9, y: -20 }} // Smooth vanish when completed
              transition={{ duration: 0.3 }}
              className="bg-zinc-800 border-t-4 border-amber-500 rounded-xl shadow-2xl overflow-hidden flex flex-col"
            >
              {/* Ticket Header */}
              <div className="bg-zinc-900/50 p-5 border-b border-zinc-700/50 flex justify-between items-start">
                ...
                <div className="text-right">
                  <p className="text-zinc-400 text-sm">{new Date(order.orderTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                ...
                <ul className="space-y-3">
                  {order.orderItems.map((oi, idx) => (
                    <li key={idx} className="flex items-center text-lg text-zinc-200 bg-zinc-900/30 p-2 rounded">
                      <span className="bg-zinc-700 text-white font-bold w-8 h-8 rounded flex items-center justify-center mr-4">
                        {oi.quantity}
                      </span> 
              ...
              <div className="p-5 bg-zinc-900/50 border-t border-zinc-700/50">
                <motion.button 
                  ...
                  className="w-full bg-amber-600 disabled:bg-zinc-600 hover:bg-amber-500 text-white font-bold py-4 rounded-lg uppercase tracking-widest transition-colors shadow-lg"
                >
