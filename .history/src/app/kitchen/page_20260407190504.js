...
export default function KitchenDisplay() {
  ...
  // 1. Fetch Menu for Dictionary & Active Orders
  useEffect(() => {
  ...
  // 2. The new 2-Step Update Function
  const updateOrderStatus = async (orderId, newStatus) => {
    setLoadingOrderId(orderId); 
    
    try {
      // Calls the status update endpoint on your C# backend
        ...
        <AnimatePresence>
          {orders.map((order) => (
            <motion.div 
              ...
              // Change the top border color depending on if it's cooking (Status 1)
              className={`bg-zinc-800 rounded-xl shadow-2xl overflow-hidden flex flex-col border-t-4 ${order.status === 1 ? 'border-red-500' : 'border-amber-500'}`}
            >
              {/* Ticket Header */}
              ...
              {/* Order Items using your menuDict */}
                ...
                <ul className="space-y-3">
                  {order.orderItems.map((oi, idx) => (
                    <li key={idx} className="flex items-center text-lg text-zinc-200 bg-zinc-900/30 p-2 rounded">
                      <span className="bg-zinc-700 text-white font-bold w-8 h-8 rounded flex items-center justify-center mr-4 shrink-0">
                        {oi.quantity}
                      </span> 
              ...
              <div className="p-5 bg-zinc-900/50 border-t border-zinc-700/50">
                {order.status === 1 ? (
                  <motion.button 
                    ...
                    className="w-full bg-green-600 disabled:bg-zinc-600 hover:bg-green-500 text-white font-bold py-4 rounded-lg uppercase tracking-widest transition-colors shadow-lg"
                  >
