        ...
        <div className="hidden print:flex justify-center bg-white text-black min-h-screen p-8 font-serif">
          ...
          <div className="w-full max-w-[400px] bg-white border border-gray-200 p-10 shadow-sm relative">
            ...
            <div className="border-y border-dashed border-gray-300 py-4 mb-8 text-xs text-gray-600 font-sans tracking-wider uppercase flex justify-between">
              <div>
                <p className="mb-1"><span className="font-bold text-gray-900">Date:</span> {receiptData.date.split(',')[0]}</p>
                <p><span className="font-bold text-gray-900">Time:</span> {receiptData.date.split(',')[1]}</p>
              </div>
              <div className="text-right">
                <p className="mb-1"><span className="font-bold text-gray-900">Table:</span> {sessionData.table}</p>
                <p><span className="font-bold text-gray-900">Guest:</span> {sessionData.name}</p>
              </div>
            </div>

            <div className="mb-8 text-xs text-gray-500 font-sans tracking-widest uppercase">
              Order #{id}
            </div>
            ...
            <div className="space-y-4 mb-10">
              {receiptData.items.map((item, idx) => (
                <div key={idx} className="flex items-end justify-between text-sm text-gray-800">
                  ...
                  {/* CSS Trick for dotted leader line */}
            ...
            <div className="border-t-2 border-black pt-4 mb-10 space-y-2 text-sm text-gray-800">
              ...
              <div className="flex justify-between">
                <span>Tax (8.875%)</span>
                <span>${(receiptData.total * 0.08875).toFixed(2)}</span>
              </div>
              <div className="flex justify-between font-bold text-lg mt-4 pt-4 border-t border-dashed border-gray-300">
                <span className="uppercase tracking-widest">Total</span>
                <span>${(receiptData.total * 1.08875).toFixed(2)}</span>
              </div>
            </div>

            {/* Signature Area */}
            <div className="mt-16 pt-8 border-t border-gray-300">
              <div className="flex justify-between text-sm text-gray-500 mb-12">
                <span>Tip:</span>
                <span className="w-48 border-b border-gray-400"></span>
              </div>
            ...
            <div className="mt-16 text-center">
              <p className="text-sm text-gray-800 italic font-serif mb-2">Thank you for dining with us.</p>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-sans">Please retain for your records</p>
            </div>
