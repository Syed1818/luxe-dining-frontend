...
// Ultra-smooth physics for elements shifting around
const fluidSpring = { type: "spring", bounce: 0.15, duration: 0.7 };
// The classic premium easing curve (rapid start, buttery slow stop)
const smoothEase = [0.22, 1, 0.36, 1];

// Staggered animation rules for the grid
const containerVariants = {
  hidden: { opacity: 0 },
  show: {
    opacity: 1,
    transition: { staggerChildren: 0.08, ease: smoothEase } // Delays each card slightly
  }
};
...
const submitOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Parse the response to get the newly created Order ID
        const createdOrder = await res.json();
        const newOrderId = createdOrder.orderID || createdOrder.id; // Adjust based on your C# model
      ...
      toast.error("Network error. Kitchen unreachable.");
    }
  };
  ...
  return (
    <>
      ...
      <main className="relative z-10 min-h-screen bg-transparent text-zinc-100 font-sans selection:bg-amber-500/30 pt-32 pb-20">
        
        {/* Header & Navigation */}
        <div className="max-w-[1600px] mx-auto px-6 mb-12">
          {/* Smooth drop-in for the header */}
          <motion.div 
          ...
          {/* Smooth fade-in for the categories */}
          <motion.div 
            initial={{ opacity: 0, x: -30 }} 
            animate={{ opacity: 1, x: 0 }} 
            transition={{ duration: 0.8, delay: 0.2, ease: smoothEase }}
            className="mt-10 flex gap-3 overflow-x-auto pb-4 scrollbar-hide border-b border-white/10"
          >
            {categories.map((cat) => (
              <button 
                ...
                className={`relative px-6 py-2 rounded-full text-xs tracking-widest uppercase transition-all whitespace-nowrap overflow-hidden ${
                  ...
                  ? "text-black font-bold shadow-[0_0_15px_rgba(245,158,11,0.5)]" 
                  : "bg-black/20 border border-white/10 text-zinc-300 hover:bg-black/40 hover:text-white backdrop-blur-md"
                }`}
              >
                {/* Fluid spring animation for the active pill background */}
                {selectedCategory === cat && (
                  <motion.div 
        ...
        <div className="max-w-[1600px] mx-auto px-6 flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-grow">
            {/* Using motion.div with staggered variants */}
            <motion.div 
              layout 
              variants={containerVariants}
              initial="hidden"
              animate="show"
              key={selectedCategory} // Forces the stagger animation to re-run when category changes
              ...
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item) => (
                  <motion.div 
                    ...
                    key={item.itemID}
                    transition={fluidSpring}
                    className="group flex bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden hover:bg-white/[0.08] hover:border-white/30 transition-colors duration-500 h-40 shadow-lg"
                  >
                    <div className="w-1/3 h-full overflow-hidden shrink-0 border-r border-white/10 relative">
                      <img 
                        ...
                        alt={item.name} 
                      />
                      {/* Subtle dark gradient overlay on image hover for premium feel */}
                    ...
                    <div className="flex-grow p-5 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-zinc-300 text-xs mt-1 line-clamp-2 leading-relaxed max-w-[280px] drop-shadow-md">
                          {item.description}
                        </p>
                      </div>
                      ...
                      <motion.button 
                        ...
                        className="mt-auto self-start text-[10px] uppercase tracking-[0.2em] font-bold text-white/60 hover:text-white flex items-center gap-2 transition-colors duration-300"
                      >
          ...
          <div className="w-full lg:w-[400px] flex-shrink-0">
            <motion.div 
              ...
              className="sticky top-32 p-8 rounded-[2rem] bg-black/30 backdrop-blur-2xl border border-white/10 shadow-2xl"
            >
              <h2 className="text-xl font-serif mb-6 text-white flex justify-between items-center drop-shadow-md">
                <span>Your Check</span>
                <span className="text-[10px] tracking-[0.2em] text-zinc-300 uppercase">{cart.length} Items</span>
              </h2>
                ...
                <AnimatePresence mode="popLayout">
                  {cart.length === 0 && (
                    <motion.p 
                      ...
                      className="text-zinc-400 text-center py-10 italic text-sm drop-shadow-md"
                    >
                  ...
                  {cart.map((item) => (
                    ...
                    >
                      ...
                      <motion.button 
                        ...
                        className="text-zinc-400 hover:text-red-400 transition-colors w-8 h-8 flex items-center justify-center rounded-full"
                      >
                        ✕
                      </motion.button>
                    </motion.div>
              ...
              <motion.button 
                ...
                className="w-full py-4 rounded-xl bg-amber-500 disabled:bg-zinc-800/50 disabled:text-zinc-500 text-black font-bold uppercase tracking-widest transition-all shadow-lg"
              >
                Place Order
              </motion.button>
            </motion.div>
