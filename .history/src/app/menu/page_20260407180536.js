  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[-10] overflow-hidden pointer-events-none">
        ...
        {/* Protective gradient overlay for contrast */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#050505]/80 via-[#050505]/70 to-[#050505]/95 backdrop-blur-sm" />
      </div>

      {/* 2. Main Content Wrapper - Ensure bg-transparent so video shows through */}
      <main className="relative z-10 min-h-screen bg-transparent text-zinc-100 font-sans selection:bg-amber-500/30 pt-32 pb-20">
            ...
            {categories.map((cat) => (
              <button 
                ...
                className={`px-6 py-2 rounded-full text-xs tracking-widest uppercase transition-all whitespace-nowrap border ${
                  ...
                  : "bg-black/30 border-white/10 text-zinc-400 hover:border-white/30 hover:text-white backdrop-blur-md"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="max-w-[1600px] mx-auto px-6 flex flex-col lg:flex-row gap-12">
              ...
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item) => (
                  <motion.div 
                    ...
                    className="group flex bg-black/40 backdrop-blur-lg border border-white/10 rounded-2xl overflow-hidden hover:bg-black/60 hover:border-white/20 transition-all h-40"
                  >
                    ...
                    <div className="flex-grow p-5 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-zinc-400 text-xs mt-1 line-clamp-2 leading-relaxed max-w-[280px]">
                          {item.description}
                        </p>
                      </div>
                      ...
                      <button 
                        ...
                        className="mt-auto self-start text-[10px] uppercase tracking-[0.2em] font-bold text-white/50 hover:text-white flex items-center gap-2 transition-colors"
                      >
          ...
          <div className="w-full lg:w-[400px] flex-shrink-0">
            <div className="sticky top-32 p-8 rounded-[2rem] bg-black/60 backdrop-blur-2xl border border-white/10 shadow-2xl">
              <h2 className="text-xl font-serif mb-6 text-white flex justify-between items-center">
                <span>Your Check</span>
                <span className="text-[10px] tracking-[0.2em] text-zinc-400 uppercase">{cart.length} Items</span>
              </h2>
              
              <div className="min-h-[100px] max-h-[40vh] overflow-y-auto pr-2 space-y-3 mb-6 scrollbar-hide">
                <AnimatePresence mode="popLayout">
                  {cart.length === 0 && (
                    <p className="text-zinc-500 text-center py-10 italic text-sm">Select items from the catalog...</p>
                  )}
                  {cart.map((item) => (
                    <motion.div 
                    ...
                    >
                      ...
                      <button onClick={() => removeFromCart(item.cartId)} className="text-zinc-400 hover:text-red-400">✕</button>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
              ...
              <div className="border-t border-white/10 pt-6 mb-8">
                <div className="flex justify-between items-end">
                  <span className="text-zinc-400 text-xs tracking-widest uppercase">Total Bill</span>
